...
// displayName rides along as auth signup metadata (raw_user_meta_data) —
// a database trigger (see sql/schema.sql's handle_new_user) copies it into
// a real public.profiles row the moment the auth user is created, which is
// what every page's greeting/assistant prompt reads from afterward instead
// of a hardcoded name.
Cal.signUp = function (email, password, displayName) {
...
// Real per-user replacement for the prototype's hardcoded "Benny" persona.
// Every page that used to just write the literal string "Benny" (index.html's
// greeting, bot.html's system prompt + greeting) now calls this on mount and
// uses whatever comes back instead. Falls back to the email's local part if
// the profiles row is missing or blank (e.g. a user created before the
// signup-metadata trigger existed), and to 'there' as a last resort — this
// should never throw or block a page from rendering.
Cal.getDisplayName = async function () {
  try {
    const user = await Cal.getUser();
    if (!user) return '';
    const { data, error } = await _withTimeout(
      _sb.from('profiles').select('display_name').eq('id', user.id).maybeSingle(),
      8000, 'getDisplayName'
    );
    if (error) console.error('getDisplayName:', error.message);
    const name = data && data.display_name;
    if (name) return name;
    return (user.email || '').split('@')[0] || 'there';
  } catch (e) {
    console.error('getDisplayName:', e.message);
    return 'there';
  }
};
...
// Normalizes text for the dedup check below — case/whitespace/trailing-
// punctuation differences (e.g. an event's own inline "Pack bag" vs. the
// same item restated in prose as "Pack your bag.") shouldn't count as a
// genuinely new item just because the exact characters don't match.
function normTaskText(s) {
  return (s || '').trim().toLowerCase().replace(/[.!?]+$/, '');
}
Cal.addTasks = async function (eventId, texts) {
  const clean = (texts || []).filter(Boolean);
  if (!clean.length) return false;
  const existing = (await Cal.loadTasks()).filter(t => t.eventId === eventId).map(t => normTaskText(t.text));
  const fresh = clean.filter(text => existing.indexOf(normTaskText(text)) === -1);
